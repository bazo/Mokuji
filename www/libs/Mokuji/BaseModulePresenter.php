...
class AdminBaseModulePresenter extends Admin_SecurePresenter{
    ...
    public function createComponentModuleCss($name)
    {
        $css = parent::createComponentCss($name);
        $css->sourcePath = dirname($this->reflection->getFileName()).'/../css' ;
        return $css;
    }

    public function createComponentModuleJs($name)
    {
       $js = parent::createComponentJs($name);
       $js->sourcePath = dirname($this->reflection->getFileName()).'/../js' ;
       return $js;
    }
}
?>
