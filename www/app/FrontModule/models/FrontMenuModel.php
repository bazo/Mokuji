...
class Front_MenuModel extends Front_BaseModel
{
	...
	{
		return db::select('c.`id`, c.`title`, c.`url`, c.parent as `parent_id`, p.title as `parent`, c.level, c.position, c.link_type')->from('[:items:] c')->leftJoin('[:items:] p')->on('c.parent = p.id')->where('c.menu_id = %i', $menu_id)->orderBy('c.level')->orderBy('c.position asc')->fetchAll();
	}
