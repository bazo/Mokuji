...
class Admin_PagesPresenter extends Admin_SecurePresenter
{
    ...
    public function actionDefault()
    {
        ...
        $this->template->link = $this->createComponentLink();
    }
    
    public function createComponentLink()
    {
    ...
    public function formNewPagePreview(Button $button)
    {
        ...
        try{
            ...
            $request = new PresenterRequest('Front:Page', 'POST', array('action' => 'preview'), array('values' => $values));
            $presenter = new Front_PagePresenter();
            $response = $presenter->run($request); //ak by bolo $this, zmeni sa aktualny presenter a cele to prestane fungovat
    ...
    public function handleBrowseFiles($dir,$ext)
    {
        ...
        if(!isset($cache[$ckey]))
        {
            ...
            $cache->save($ckey, $response, array(
                'expire' => time() + 60 * 10));
        }
        echo $cache[$ckey];
        $this->terminate();
